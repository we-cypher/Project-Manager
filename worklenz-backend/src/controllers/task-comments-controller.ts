...
export default class TaskCommentsController extends WorklenzControllerBase {
  ...
  private static async sendMail(config: IMailConfig) {
    ...
    const taskUrl = `${getBaseUrl()}/projects/${config.projectId}?tab=tasks-list&task=${config.taskId}&focus=comments`;
    const settingsUrl = `${getBaseUrl()}/settings/notifications`;
